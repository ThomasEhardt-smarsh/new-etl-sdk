...
public class TextObjectWriter extends AbstractXmlWriter<TextObject> {
    ...
    public void write(TextObject textObject) throws XMLStreamException {
        ...
        // OPTIONAL
        if (Objects.nonNull(textObject.getLexiconHits()) && !textObject.getLexiconHits().getLexiconHit().isEmpty()) {
            writeStartElement("lexicon-hits", ITM);

            var lexiconHitWriter = new LexiconHitWriter(xmlStreamWriter);
            for (LexiconHit lexiconHit : textObject.getLexiconHits().getLexiconHit()) {
