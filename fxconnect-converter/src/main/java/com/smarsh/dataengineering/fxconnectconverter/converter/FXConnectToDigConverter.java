package com.smarsh.dataengineering.fxconnectconverter.converter;

import com.smarsh.dataengineering.conversionsdk.util.AbstractEmailParser;
...
import com.smarsh.dataengineering.digconverter.writer.TranscriptWriter;
import com.smarsh.dataengineering.model.MimeType;
import com.smarsh.dataengineering.model.TextEventType;
...
import static com.smarsh.dataengineering.conversionsdk.util.AbstractEmailParser.EmailParseException;
import static com.smarsh.dataengineering.conversionsdk.util.AbstractEmailParser.StandardEmailHeader;

// NOTE: at this time, we do not have the means to dejournal an email, so this must first be done outside of here
public class FXConnectToDigConverter {
    private static final Logger log = LoggerFactory.getLogger(FXConnectToDigConverter.class);

    public final static Collection<DateTimeFormatter> DATE_TIME_FORMATTER_COLLECTION = List.of(
    ...
    final static String internalDomain = "credit-suisse.com";
    final static String endpointId = "test";
    ...
    private void convert(InputStream fxConnectInputStream, OutputStream digXmlOutputStream) throws Exception {
        AbstractEmailParser emailParser;
    ...
    protected TimeFrame getTimeFrame(Element tableBody) {
        ...
        tableBody.select("tr").stream()
                ...
                .map(Element::text)
                .map(s -> DateTimeUtils.toEpochMillis(s, DATE_TIME_FORMATTER_COLLECTION, 0))
                .forEach(aLong -> {
